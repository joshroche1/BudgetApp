...
public class AccountResource {
  ...
  public Uni<List<AccountEntity>> getAll() {
  ...
  @Consumes(MediaType.APPLICATION_JSON)
  @Produces(MediaType.APPLICATION_JSON)
  public Uni<Response> update(Long id, AccountEntity tEntity) {
    if (tEntity == null || tEntity.id == null) {
      throw new WebApplicationException("Entity not found.", 422);
    }
    return Panache.withTransaction(() -> AccountEntity.<AccountEntity> findById(id).onItem().ifNotNull().invoke(entity -> entity.id = tEntity.id)).onItem().ifNotNull().transform(entity -> Response.ok(entity).build()).onItem().ifNull().continueWith(Response.ok().status(NOT_FOUND)::build);
  }
  ...
  public Uni<Response> delete(Long id) {
    return Panache.withTransaction(() -> AccountEntity.deleteById(id)).map(deleted -> deleted ? Response.ok().status(NO_CONTENT).build() : Response.ok().status(NOT_FOUND).build());
  }
