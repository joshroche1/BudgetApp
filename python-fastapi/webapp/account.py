...
def add_account(db: Session, newaccount):
  account = models.Account(name=newaccount['name'], accounttype=newaccount['accounttype'], currency=newaccount['currency'], iban=newaccount['iban'], bic=newaccount['bic'], country=newaccount['country'])
  db.add(account)
  db.commit()
  return account
...
def update_account_field(db: Session, acctid, field, newvalue):
  ...
  if field == "name": 
    ...
    account.iban = newvalue
    db.commit()
  elif field == "bic": 
    account.bic = newvalue
    db.commit()
  elif field == "country": 
